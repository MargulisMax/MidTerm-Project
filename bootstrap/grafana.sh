...
sudo consul agent -bind `ec2-metadata -o| cut -d " " -f2` -data-dir /var/lib/consul -config-dir /etc/consul.d -node=Grafana -retry-join "provider=aws tag_key=Name tag_value=MTP-Consul" &
